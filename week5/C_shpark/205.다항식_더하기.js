...
function solution(polynomial) {    
    ...
    polynomialArr.forEach(el => {
        ...
        if (el.includes('x')) {
            //x가 있다면, 정수가 붙었을때는 정수인 부분(3x = 3), 정수인 부분이 없다면 1을 더함 (x = 1)
            linear += Number.isInteger(num) ? num : 1;
        } else {
            //x가 없다면 숫자인 부분에 그만큼을 더함
            constant += num;
        }
    })

    let answer = '';
    //linear가 0이 아닐때 x 나 `${linear}x`를 출력
    if (linear !== 0) answer = (linear === 1) ? 'x' : `${linear}x`;
    //동일하게 answer가 비어있다면 constant를, 버어있지 않다면 `+ ${constant}`를 출력
